    ...
    const similarProducts = await productCollection.find({ 
        $and: [
            ...
            { _id: { $ne: req.params.id } }
                ...
                let reviewDocuments = await reviewCollection.find().populate({
                    path: "customer",
                    select: "username photo",
                });
        
                // Filter and aggregate reviews for the specific product
                let reviews = [];
                reviewDocuments.forEach(doc => {
                    doc.reviews.forEach(reviewItem => {
                        if (reviewItem.product.toString() === productDetails._id.toString()) {
                            // Attach the customer data to the review
                            reviewItem.customer = doc.customer; 
